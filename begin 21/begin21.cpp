...
{
    float x1,y1,x2,y2,x3,y3,p,s12,s23,s13,S;
    ...
    s12 = sqrt(pow((x2 - x1), 2) + pow((y2 - y1), 2));//подсчет сторон треугольника по формуле из задачи 20
    s23 = sqrt(pow((y2 - y3), 2) + pow((x2 - x3), 2));
    s13 = sqrt(pow((y1 - y3), 2) + pow((x1 - x3), 2));

    p = (s12 + s23 + s13)/2;

    S = sqrt(p * (p - s12) * (p - s13) * (p - s23));//подсчет площади по формуле герона
